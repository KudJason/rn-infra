...
if ! command -v cloudflared >/dev/null 2>&1; then
  ...
  if [ -n "$${CF_ARCH}" ]; then
    ...
    if curl -fsSL "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-$${CF_ARCH}.deb" -o "$${CF_DEB}"; then
      dpkg -i "$${CF_DEB}" || apt-get install -f -y && dpkg -i "$${CF_DEB}"
      rm -f "$${CF_DEB}"
    fi
  fi
fi
...
while true; do
  CURRENT=$(kubectl get nodes --no-headers 2>/dev/null | grep -c "Ready" || true)
  echo "Ready nodes: $CURRENT / $TARGET_NODE_COUNT"
  [ "$CURRENT" -ge "$TARGET_NODE_COUNT" ] && break
  [ $(( $(date +%s) - START_TIME )) -gt $MAX_WAIT ] && break
  sleep 10
done
...
# Backup script
cat >/usr/local/bin/rn_prod_backup.sh <<'BACKUP'
#!/bin/bash
set -e
DATE=$(date +%F)
BK_DIR=/tmp/rn_prod_bk_$DATE
mkdir -p "$BK_DIR"
POD=$(kubectl get pod -n ruralneighbour-prod -l app=postgis-pg-prod -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)
if [ -n "$POD" ]; then
  kubectl exec -n ruralneighbour-prod $POD -- pg_dumpall -U postgres > "$BK_DIR/postgres-$DATE.sql"
  gzip "$BK_DIR/postgres-$DATE.sql"
  gsutil cp "$BK_DIR/postgres-$DATE.sql.gz" gs://${var.prod_backup_bucket_name}/postgres/ || true
fi
rm -rf "$BK_DIR"
BACKUP
chmod +x /usr/local/bin/rn_prod_backup.sh
(crontab -l 2>/dev/null; echo "0 3 * * * /usr/local/bin/rn_prod_backup.sh > /var/log/rn_prod_backup.log 2>&1") | crontab -

echo "=== Production VM Ready ==="
echo "View pods: kubectl get pods -n ruralneighbour-prod"
